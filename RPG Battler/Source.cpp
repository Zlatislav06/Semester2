...
void free(User* player1, User* player2)
{
	player1 = nullptr;
	player2 = nullptr;
	delete player1;
...
User* LogInOrCreateInGame(User* player,int indexOfPlayer, vector<std::unique_ptr<User>>& users,bool check)
{
	...
	if (command == "1" || command == "1)" || ToLower(command) == "create")
	{
		...
		while (true)
		{
			...
			try
			{
				cout << "Enter valid password - minimum 8 characters (uppercase letter,lowercase letter, special character and number):";
				cin >> password;
...
PlayerCharacter* ChooseHeroOneFromMoreThenOne(PlayerCharacter* playerHero,User* player,auto& playerCharacters,int size)
{
	...
	auto it = std::find_if(playerCharacters.begin(), playerCharacters.end(), [&](const std::unique_ptr<PlayerCharacter>& x)
		{
			return x->getName() == choice || ConvertFromHeroTypeToString(x->getHeroType())==choice;
		});
...
void MoveUseItemsP1VsP2(User* player1, User* player2, PlayerCharacter* player1Hero, PlayerCharacter* player2Hero)
{
	while (true)
	{
		...
		cout << "4) Exit (you lost your move)" << endl;
		cin >> command;
		try
		{
			...
			else if (command == "4" || command == "4)" || ToLower(command) == "exit")
			{
				break;
			}
			else
...
void MoveP1VsP2(User* player1, User* player2, PlayerCharacter* player1Hero, PlayerCharacter* player2Hero)
{
	while (true)
	{
		try
		{
			...
			cout << "3) Information for items, which you have (info)" << endl;
			...
			else if (command == "3" || command == "3)" || ToLower(command) == "info")
			{
				cout << endl;
				player1->PrintItems();
				cout << endl;
	...
	while (true)
	{
		try
		{
			...
			else if (ready == "No" || ready == "no" || ready == "n")
			{
				...
				ConsoleClear();
				setColor(Color::Yellow);
				cout << "Good!\nI will wait "<<time<<" seconds and ask again!" << endl;
				setColor(Color::White);
...
void ChooseOneOfMainOptions(User* player1, User* player2, vector<std::unique_ptr<User>>& users,bool check)
{
	while (true)
	{
		try
		{
			...
			if (command == "1" || command == "1)" || ToLower(command) == "pvp")
			{
				while (true)
				{
					try
					{
						...
						if (player1->getName() == player2->getName() && player1->getPassword() == player2->getPassword())
						{
							throw Exceptions("Haha nice try but you can't have the same user account for two players!");
						}
						else
						{
							break;
			...
			else if (command == "5" || command == "5)" || ToLower(command) == "ranklist")
			{
				cout << endl;
				setColor(Color::LightBlue);
				player1->RankList();
			...
			else if (command == "6" || command == "6)" || ToLower(command) == "exit")
			{
				free(player1, player2);
				break;
			}
...
void InformationForItems()
{
	cout << "The types of magic items are:" << endl;
	setColor(Color::Green);
	cout << "Healing Potion (30 XP): " << endl;
	setColor(Color::White);
	cout << "Adds a random number between 5 and 10 to a character's current HP." << endl;
	cout << "If this would cause the character's current HP to exceed their maximum, " << endl;
	cout << "the character instead gains their maximum current HP." << endl;
	setColor(Color::Red);
	cout << "Blade (50 XP): " << endl;
	setColor(Color::White);
	cout << "The character's next attack (on the next turn) will double the Damage the character deals to the opponent." << endl;
	cout << "For example: a player activates a Blade." << endl;
	cout << "On their next turn, they attack and deal 4 Damage. " << endl;
	cout << "Due to the Blade, the damage is doubled to 8." << endl;
	setColor(Color::LightPurple);
	cout << "Mirror (80 XP): " << endl;
	setColor(Color::White);
	cout << "Blocks the next use of the enemy character's special ability. " << endl;
	cout << "Can be removed by using a Ray." << endl;
	setColor(Color::Yellow);
	cout << "Ray (90 XP):" << endl;
	setColor(Color::White);
	cout << "Removes the effect of a Mirror if it is used against the player." << endl;
	cout << "The ray is special - it can be used directly after the opponent uses a Mirror, and does not count as part of the player's turn using it, i.e. a player using a Ray to defend against a Mirror can then play their turn normally." << endl;
	setColor(Color::Blue);
	cout << "Shield (100 XP): " << endl;
	setColor(Color::White);
	cout << "Blocks Damage from an opponent's attack." << endl;
	cout << "The Shield is special - it can be used directly after the opponent deals Damage to the character, " << endl;
	cout << "and does not count as part of the player's turn using it, i.e.a player using a Shield to defend against Damage can then play their turn normally." << endl<<endl;
}
void Information()
{
	string command = "";
